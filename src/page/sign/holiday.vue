            ...
            <div class="layui-field-box"  v-show="menuShow">
                <div class="layui-box-searchber">
                    ...
                    <form class="layui-form layui-form-pane">
                        ...
                        <div class="layui-inline" >
                            <div @click="daoEvent()" class="dao" v-if="chang('/master/rest/overtimeExcel')">导出</div>
                        </div>
                ...
                <table class="layui-table admin-table">
                    ...
                    <tr v-for="(item,index) in signList" :key="index">
                        ...
                        <td>{{item.days}}天</td>
                        <td>{{item.clearing_from1}}</td>
                        <td>{{item.reason}}</td>
                        <td>{{item.exdate}}</td>
                        <td>{{item.status}}</td>
                        <td>{{item.exname}}</td>
                        <td>{{item.remark}}</td>
                        <td  v-show="chang('/master/rest/examine')">
...
//             请假管理
                me.$axios.post(me.url+"master/rest/allRest",formData)
                    .then(function (result) {
                        ...
                        me.signList.forEach(function(index){
                            ...
                            me.cleartype.forEach(function(item){
                                if(index.clearing_from==item.id){
                                    index.clearing_from1=item.type
                                }
                            })
                        })
            ...
            daoEvent(){
                ...
                if(me.signList.length ==0||me.signList.length ==undefined){
                    ...
                    me.$axios({ // 用axios发送post请求
                        method: 'post',
                        url: me.url+"/master/rest/overtimeExcel", // 请求地址
                        data:{
