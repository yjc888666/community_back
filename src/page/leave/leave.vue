            ...
            <div class="layui-field-box"  v-show="menuShow">
                <table class="layui-table admin-table">
                    ...
                    <tr v-for="(item,index) in signList" :key="index">
                        ...
                        <td>{{item.remark}}</td>
                        <td>
                            <a href="javascript:;" class="elementdel layui-btn layui-btn-danger layui-btn-sm ajax-delete" @click="leaveMenu(item.id,item.myname)" v-if="item.status!=0" v-show="chang('/master/leave/examine')">审核</a>
                        </td>
                    </tr>
...
//             请假管理
                me.$axios.post(me.url+"master/leave/all?page=" + this.pageNum + "&size=" + this.pageSize)
                    .then(function (result) {
