            ...
            <div class="layui-field-box"  v-show="menuShow">
                <table class="layui-table admin-table">
                    ...
                    <tr v-for="(item,index) in signList" :key="index">
                        ...
                        <td>{{item.reimbursedate}}</td>
                        <td>{{item.reimburse_money}}元</td>
                        <td>{{item.reimburse_content}}</td>
                        <td>{{item.exdate}}</td>
                        <td>{{item.status}}</td>
                        <td>{{item.exname}}</td>
                        <td>{{item.remark}}</td>
                        <td>
                            <a href="javascript:;" class="elementdel layui-btn layui-btn-danger layui-btn-sm ajax-delete" @click="leaveMenu(item.id,item.myname)" v-if="item.status!=0" v-show="chang('/master/reimbursement/examine')">审核</a>
                        </td>
                    </tr>
                                ...
                                <div class="layui-form-item" style="margin-bottom:0">
                                    <div class="layui-input-inline">
                                        <el-form-item label="报销人" prop="name">
                                            <el-input v-model="ruleFormAdd.name" placeholder="请输入报销人" :disabled="true"></el-input>
                                        </el-form-item>
                                    </div>
                                </div>
                                ...
                                <div class="layui-form-item">
                                    <div class="layui-input-inline">
                                        <el-form-item label="报销日期" prop="reimbursedate">
                                            <el-date-picker
                                                v-model="ruleFormAdd.reimbursedate"
                                                type="date"
                                ...
                                <div class="btable-paged">
                                    <div class="layui-main">
                                        <div class="formbtngroup">
                                            <el-form-item>
                                                <el-button type="primary" @click="submitFormAdd('ruleFormAdd')" v-show="chang('/master/reimbursement/examine')">完成</el-button>
...
//           权限的接口
            ...
            signEvent(){
                var me=this;
                me.$axios.post(me.url+"master/reimbursement/all?page=" + this.pageNum + "&size=" + this.pageSize)
                    .then(function (result) {
            ...
            submitFormAdd(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        me.$axios.post(me.url+"master/reimbursement/examine", me.ruleFormAdd)
                            .then(function (result) {
